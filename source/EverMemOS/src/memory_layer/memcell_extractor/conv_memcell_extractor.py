...
class ConvMemCellExtractor(MemCellExtractor):
    ...
    async def extract_memcell(
        ...
        # === Force extract bypass (skip boundary detection entirely) ===
        if getattr(request, 'force_extract', False) and new_message_dict_list:
            combined_messages = history_message_dict_list + new_message_dict_list
            ts_value = combined_messages[-1].get("timestamp")
            timestamp = dt_from_iso_format(ts_value)
            participants = self._extract_participant_ids(combined_messages)
            summary_text = combined_messages[-1].get("content", "Force extracted")[:200]

            memcell = MemCell(
                user_id_list=request.user_id_list,
                original_data=combined_messages,
                timestamp=timestamp,
            ...
            record_boundary_detection(
                space_id=get_space_id_for_metrics(),
                raw_data_type=self.raw_data_type.value,
                result='force_extracted',
                trigger_type='force_extract',
            )
            record_memcell_extracted(
                space_id=get_space_id_for_metrics(),
                raw_data_type=self.raw_data_type.value,
            ...
            logger.info(
                f"✅ Force extract MemCell created: event_id={memcell.event_id}, "
                f"messages={len(combined_messages)}"
            )
