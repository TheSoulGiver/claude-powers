...
from core.resilience import get_breaker, CircuitOpenError

# 限制 LLM 并发调用数，防止 Ollama/远程 API 过载
_LLM_SEMAPHORE = asyncio.Semaphore(int(os.getenv("LLM_MAX_CONCURRENT", "3")))
...
class OpenAIAdapter(LLMBackendAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._breaker = get_breaker("llm_api", failure_threshold=5, recovery_timeout=30)
            ...
            async with _LLM_SEMAPHORE:
                if final_params.get("stream"):
                    ...
                    async def stream_gen():
                        response_stream = await self._breaker.call(
