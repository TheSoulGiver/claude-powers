...
from core.cache.query_cache import get_query_cache, make_cache_key
...
class MemoryManager:
    ...
    async def memorize(self, memorize_request: MemorizeRequest) -> int:
        ...
        # 写入成功后失效该用户的查询缓存
        if count > 0 and memorize_request.user_id:
            get_query_cache().invalidate_user(memorize_request.user_id)
        return count
    ...
    async def retrieve_mem(
        ...
        try:
            ...
            cache_key = make_cache_key(
                retrieve_mem_request.user_id or "",
                retrieve_mem_request.query or "",
                retrieve_method.value if retrieve_method else "unknown",
            )
            cached = cache.get(cache_key)
            if cached is not None:
                logger.info(f"retrieve_mem cache HIT: {cache_key}")
                return cached
